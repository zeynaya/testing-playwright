...
function createRemoveOutputDirsTask() {
  return {
    ...
    setup: async ({
      ...
      await Promise.all(Array.from(outputDirs).map(outputDir => (0, _server.removeFolders)([outputDir]).then(async ([error]) => {
        if (!error) return;
        if (error.code === 'EBUSY') {
          ...
          await Promise.all(entries.map(entry => (0, _server.removeFolders)([path.join(outputDir, entry)])));
        } else {
          throw error;
        }
      })));
