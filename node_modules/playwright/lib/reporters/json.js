...
class JSONReporter {
  ...
  _serializeReport(result) {
    const report = {
      config: {
        ...
        rootDir: (0, _server.toPosixPath)(this.config.rootDir),
        projects: this.config.projects.map(project => {
          return {
            outputDir: (0, _server.toPosixPath)(project.outputDir),
            repeatEach: project.repeatEach,
            retries: project.retries,
            metadata: project.metadata,
            id: (0, _config.getProjectId)(project),
            name: project.name,
            testDir: (0, _server.toPosixPath)(project.testDir),
  ...
  _relativeLocation(location) {
    ...
    return {
      file: (0, _server.toPosixPath)(path.relative(this.config.rootDir, location.file)),
      line: location.line,
  ...
  _mergeTestsFromSuite(to, from) {
    ...
    for (const spec of from.specs || []) {
      const toSpec = to.specs.find(s => s.title === spec.title && s.file === (0, _server.toPosixPath)(path.relative(this.config.rootDir, spec.file)) && s.line === spec.line && s.column === spec.column);
