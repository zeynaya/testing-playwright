...
`);
  ...
  async onEnd(result) {
    ...
    await (0, _server.removeFolders)([this._outputFolder]);
...
async function showHTMLReport(reportFolder, host = 'localhost', port, testId) {
  ...
  try {
    ...
    (0, _server.gracefullyProcessExitDoNotHang)(1);
    return;
  }
...
class HtmlBuilder {
  ...
  async build(metadata, projectSuites, result, topLevelErrors) {
    ...
    for (const projectSuite of projectSuites) {
      for (const fileSuite of projectSuite.suites) {
        ...
        const fileId = (0, _utils.calculateSha1)((0, _server.toPosixPath)(fileName)).slice(0, 20);
    ...
    await (0, _server.copyFileAndMakeWritable)(path.join(appFolder, 'index.html'), path.join(this._reportFolder, 'index.html'));
    ...
    if (this._hasTraces) {
      ...
      for (const file of fs.readdirSync(traceViewerFolder)) {
        ...
        await (0, _server.copyFileAndMakeWritable)(path.join(traceViewerFolder, file), path.join(traceViewerTargetFolder, file));
      }
      for (const file of fs.readdirSync(path.join(traceViewerFolder, 'assets'))) {
        if (file.endsWith('.map') || file.includes('xtermModule')) continue;
        await (0, _server.copyFileAndMakeWritable)(path.join(traceViewerFolder, 'assets', file), path.join(traceViewerAssetsTargetFolder, file));
      }
    }
  ...
  _serializeAttachments(attachments) {
    ...
    return attachments.map(a => {
      ...
      if (a.body instanceof Buffer) {
        ...
        const extension = (0, _server.sanitizeForFilePath)(path.extname(a.name).replace(/^\./, '')) || _utilsBundle.mime.getExtension(a.contentType) || 'dat';
  ...
  _relativeLocation(location) {
    ...
    const file = (0, _server.toPosixPath)(path.relative(this._config.rootDir, location.file));
    return {
      file,
      line: location.line,
