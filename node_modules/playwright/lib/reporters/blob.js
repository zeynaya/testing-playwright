...
class BlobReporter extends _teleEmitter.TeleReporterEmitter {
  ...
  async _prepareOutputFile() {
    ...
    if (!process.env.PWTEST_BLOB_DO_NOT_REMOVE) await (0, _server.removeFolders)([outputDir]);
  ...
  _defaultReportName(config) {
    ...
    if (this._options._commandHash) reportName += '-' + (0, _server.sanitizeForFilePath)(this._options._commandHash);
    if (config.shard) {
