...
async function toHaveURL(page, expected, options) {
  ...
  if (!(typeof expected === 'string') && !(expected && 'test' in expected && typeof expected.test === 'function') && !(typeof expected === 'function')) {
    throw new Error([
    // Always display `expected` in expectation place
    (0, _matcherHint.matcherHint)(this, undefined, matcherName, expression, undefined, matcherOptions), `${_utilsBundle.colors.bold('Matcher error')}: ${(0, _expectBundle.EXPECTED_COLOR)('expected')} value must be a string, regular expression, or predicate`, this.utils.printWithType('Expected', expected, this.utils.printExpected)].join('\n\n'));
  }
  ...
  try {
    await page.mainFrame().waitForURL(url => {
      ...
      if (options !== null && options !== void 0 && options.ignoreCase) {
        return !this.isNot === (0, _utils.urlMatches)(baseURL === null || baseURL === void 0 ? void 0 : baseURL.toLocaleLowerCase(), lastCheckedURLString.toLocaleLowerCase(), typeof expected === 'string' ? expected.toLocaleLowerCase() : expected);
      }
  ...
  return {
    name: matcherName,
    pass: this.isNot,
    message: () => toHaveURLMessage(this, matcherName, expression, typeof expected === 'string' ? (0, _utils.constructURLBasedOnBaseURL)(baseURL, expected) : expected, lastCheckedURLString, this.isNot, true, timeout),
