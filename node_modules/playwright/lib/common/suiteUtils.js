...
function bindFileSuiteToProject(project, suite) {
  ...
  const fileId = (0, _utils.calculateSha1)((0, _server.toPosixPath)(relativeFile)).slice(0, 20);
  ...
  result.forEachTest((test, suite) => {
    ...
    const testIdExpression = `[project=${project.id}]${(0, _server.toPosixPath)(file)}\x1e${titles.join('\x1e')}`;
...
function applyRepeatEachIndex(project, fileSuite, repeatEachIndex) {
  ...
  fileSuite.forEachTest((test, suite) => {
    if (repeatEachIndex) {
      const [file, ...titles] = test.titlePath();
      const testIdExpression = `[project=${project.id}]${(0, _server.toPosixPath)(file)}\x1e${titles.join('\x1e')} (repeat:${repeatEachIndex})`;
