...
async function launchContext(options, extraOptions) {
  ...
  if (!extraOptions.headless) contextOptions.deviceScaleFactor = os.platform() === 'darwin' ? 2 : 1;
  ...
  if (options.saveHar) {
    contextOptions.recordHar = {
      path: path.resolve(process.cwd(), options.saveHar),
      mode: 'minimal'
    };
  ...
  process.on('SIGINT', async () => {
    await closeBrowser();
    (0, _server.gracefullyProcessExitDoNotHang)(130);
  });
