...
async function downloadBrowserWithProgressBar(title, browserDirectory, executablePath, downloadURLs, downloadFileName, downloadConnectionTimeout) {
  ...
  try {
    ...
    for (let attempt = 1; attempt <= retryCount; ++attempt) {
      ...
      if (await (0, _fileUtils.existsAsync)(zipPath)) await fs.promises.unlink(zipPath);
      if (await (0, _fileUtils.existsAsync)(browserDirectory)) await fs.promises.rmdir(browserDirectory, {
        recursive: true
      });
...
function downloadBrowserWithProgressBarOutOfProcess(title, browserDirectory, url, zipPath, executablePath, connectionTimeout) {
  ...
  cp.on('exit', code => {
    ...
    if (!fs.existsSync((0, _.browserDirectoryToMarkerFilePath)(browserDirectory))) promise.resolve({
