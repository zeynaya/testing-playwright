...
const registryDirectory = exports.registryDirectory = (() => {
  ...
  if (!path.isAbsolute(result)) {
    ...
    result = path.resolve((0, _utils.getFromENV)('INIT_CWD') || process.cwd(), result);
  }
  return result;
})();
...
class Registry {
  constructor(browsersJSON) {
    ...
    const webkitLinuxLddDirectories = [path.join('minibrowser-gtk'), path.join('minibrowser-gtk', 'bin'), path.join('minibrowser-gtk', 'lib'), path.join('minibrowser-gtk', 'sys', 'lib'), path.join('minibrowser-wpe'), path.join('minibrowser-wpe', 'bin'), path.join('minibrowser-wpe', 'lib'), path.join('minibrowser-wpe', 'sys', 'lib')];
    this._executables.push({
      type: 'browser',
      name: 'webkit',
      browserName: 'webkit',
  ...
  _createBidiFirefoxChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      if (found.length === 1) return path.join(folder, found[0], suffix);
      if (found.length > 1) {
  ...
  async _validateHostRequirements(sdkLanguage, browserDirectory, linuxLddDirectories, dlOpenLibraries, windowsExeAndDllDirectories) {
    if (os.platform() === 'linux') return await (0, _dependencies.validateDependenciesLinux)(sdkLanguage, linuxLddDirectories.map(d => path.join(browserDirectory, d)), dlOpenLibraries);
    if (os.platform() === 'win32' && os.arch() === 'x64') return await (0, _dependencies.validateDependenciesWindows)(sdkLanguage, windowsExeAndDllDirectories.map(d => path.join(browserDirectory, d)));
  }
  ...
  async install(executablesToInstall, forceReinstall) {
    ...
    try {
      ...
      await fs.promises.writeFile(path.join(linksDir, (0, _utils.calculateSha1)(PACKAGE_PATH)), PACKAGE_PATH);
  ...
  async uninstall(all) {
    ...
    if (all) {
      ...
      for (const link of links) await fs.promises.unlink(path.join(linksDir, link));
    } else {
      await fs.promises.unlink(path.join(linksDir, (0, _utils.calculateSha1)(PACKAGE_PATH))).catch(() => {});
    }
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    if (isPowerShell) {
      const args = ['-ExecutionPolicy', 'Bypass', '-File', path.join(BIN_PATH, scriptName), ...scriptArgs];
      const {
        code
      ...
      const {
        ...
        args,
        elevatedPermissions
      } = await (0, _dependencies.transformCommandsForRoot)([`bash "${path.join(BIN_PATH, scriptName)}" ${scriptArgs.join('')}`]);
