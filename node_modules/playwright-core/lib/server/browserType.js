...
class BrowserType extends _instrumentation.SdkObject {
  ...
  async _launchProcess(progress, options, isPersistent, browserLogsCollector, userDataDir) {
    ...
    if (userDataDir) {
      ...
      if (!(await (0, _fileUtils.existsAsync)(userDataDir))) await fs.promises.mkdir(userDataDir, {
        recursive: true,
        mode: 0o700
      });
    } else {
  ...
  async _createArtifactDirs(options) {
    if (options.downloadsPath) await fs.promises.mkdir(options.downloadsPath, {
      recursive: true
    });
    if (options.tracesDir) await fs.promises.mkdir(options.tracesDir, {
      recursive: true
    });
  }
