...
function validateTraceUrls(traceUrls) {
  for (const traceUrl of traceUrls) {
    ...
    if (!traceUrl.startsWith('http://') && !traceUrl.startsWith('https://') && !fs.existsSync(traceFile) && !fs.existsSync(traceFile + '.trace')) throw new Error(`Trace file ${traceUrl} does not exist!`);
  }
}
  ...
  server.routePrefix('/trace', (request, response) => {
    ...
    if (relativePath.startsWith('/file')) {
      try {
        ...
        if (fs.existsSync(filePath)) return server.serveFile(request, response, url.searchParams.get('path'));
...
async function installRootRedirect(server, traceUrls, options) {
  ...
  if (path.sep !== path.posix.sep) params.set('pathSeparator', path.sep);
...
async function runTraceViewerApp(traceUrls, browserName, options, exitOnClose) {
  ...
  if (exitOnClose) page.on('close', () => (0, _server.gracefullyProcessExitDoNotHang)(0));
  return page;
}
...
class StdinServer {
  constructor() {
    ...
    process.stdin.on('close', () => (0, _server.gracefullyProcessExitDoNotHang)(0));
  }
  onconnect() {}
...
function traceDescriptor(traceName) {
  ...
  if (fs.existsSync(resourcesDir)) {
    for (const name of fs.readdirSync(resourcesDir)) result.entries.push({
      name: 'resources/' + name,
