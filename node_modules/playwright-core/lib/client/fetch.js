...
var _rtti = require("../utils/rtti");
let _Symbol$asyncDispose, _Symbol$asyncDispose2;
/**
...
class APIRequestContext extends _channelOwner.ChannelOwner {
  ...
  async _innerFetch(options = {}) {
    return await this._wrapApiCall(async () => {
      ...
      (0, _debug.assert)(options.request || typeof options.url === 'string', 'First argument must be either URL string or Request');
      (0, _debug.assert)((options.data === undefined ? 0 : 1) + (options.form === undefined ? 0 : 1) + (options.multipart === undefined ? 0 : 1) <= 1, `Only one of 'data', 'form' or 'multipart' can be specified`);
      (0, _debug.assert)(options.maxRedirects === undefined || options.maxRedirects >= 0, `'maxRedirects' must be greater than or equal to '0'`);
      (0, _debug.assert)(options.maxRetries === undefined || options.maxRetries >= 0, `'maxRetries' must be greater than or equal to '0'`);
