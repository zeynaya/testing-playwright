...
class Request extends _channelOwner.ChannelOwner {
  ...
  frame() {
    if (!this._initializer.frame) {
      (0, _debug.assert)(this.serviceWorker());
...
class Route extends _channelOwner.ChannelOwner {
  ...
  async _innerFulfill(options = {}) {
    ...
    if (options.json !== undefined) {
      (0, _debug.assert)(options.body === undefined, 'Can specify either body or json parameters');
...
class RouteHandler {
  constructor(baseURL, url, handler, times = Number.MAX_SAFE_INTEGER) {
    ...
    this.url = url;
    this.handler = handler;
    this._svedZone = _zones.zones.current().without('apiZone');
  }
  static prepareInterceptionPatterns(handlers) {
    const patterns = [];
    let all = false;
